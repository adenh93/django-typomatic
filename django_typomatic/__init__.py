...
def __map_choices_to_union(field_name, choices):
    ...
    return " | ".join(
        f'"{key}"' if type(key) == str else str(key) for key in choices.keys()
    )
...
def __get_choices_escape_spec_chars(choices):
    ...
    key_string = any(
        type(key) == str and (char in key or key[0].isdigit())
        for char in spec_chars
        for key in choices.keys()
    )
    value_string = any(
        type(value) == str and (char in value or value[0].isdigit())
        for char in spec_chars
...
def __map_choices_to_enum(enum_name, choices):
    ...
    for key, value in choices.items():
        ...
        if type(key) == str and key_esc:
            choices_enum = (
                choices_enum
                + f"    '{str(key).upper().replace(' ', '_')}' = '{key}',\n"
            )
        elif type(key) == str and not key_esc:
            choices_enum = (
                choices_enum + f"    {str(key).upper().replace(' ', '_')} = '{key}',\n"
            )
        elif value_esc and key_esc and type(key) == str:
            choices_enum = (
                choices_enum
                + f"    '{str(value).upper().replace(' ', '_')}' = '{key}',\n"
            )
        elif value_esc:
            choices_enum = (
                choices_enum
            ...
            choices_enum = (
                choices_enum + f"    {str(value).upper().replace(' ', '_')} = {key},\n"
            )
...
def __map_choices_to_enum_values(enum_name, choices):
    ...
    for key, value in choices.items():
        ...
        if type(key) == str and key_esc:
            choices_enum = (
                choices_enum + f"    '{str(key).replace(' ', '_')}' = '{value}',\n"
            )
        elif type(key) == str:
            choices_enum = (
                choices_enum + f"    {str(key).replace(' ', '_')} = '{value}',\n"
...
def __process_field(
    ...
    if field_type in __serializers[context] or __is_known_serializer_type(
        field_type, context
    ):
        ts_type = __get_trimmed_name(field_type.__name__, trim_serializer_output)
    elif field_type in __field_mappings[context]:
        ts_type = __field_mappings[context].get(field_type, "any")
    elif (
        (context in __mapping_overrides)
        and (serializer in __mapping_overrides[context])
        ...
        hasattr(field, "choice_strings_to_values")
        and enum_choices
        and enum_values
        and not enum_keys
    ):
        ...
        is_many, ts_type = __get_nested_serializer_field(
            context,
            enum_choices,
            enum_values,
            enum_keys,
...
def __get_nested_serializer_field(
    context,
    enum_choices,
    enum_values,
    enum_keys,
    ...
    if (
        isinstance(return_type, list)
        or isinstance(return_type, tuple)
        or isinstance(return_type, set)
    ):
        ...
        if inspect.isclass(return_type) and issubclass(return_type, BaseSerializer):
            is_external_serializer = (
                return_type.__module__.replace(".serializers", "") != context
            )
            is_serializer_type = True

            if is_external_serializer and return_type not in __serializers.get(
                context, []
            ):
...
def __process_method_field(
    field_name, return_type, enum_choices, enum_values, enum_keys, many=False
):
    """
...
def __get_ts_interface(
    ...
    for key, value in fields:
        ts_property, ts_type = __process_field(
            key,
            value,
            context,
            serializer,
...
def generate_ts(
    ...
    with open(output_path, "w") as output_file:
        interfaces = __generate_interfaces(
            context,
            trim_serializer_output,
            camelize,
            enum_choices,
